...
const EditResume = ({ createResume, history, resume: { loading, resume } }) => {
	...
	useEffect(() => {
		...
		if (resume) {
			...
			setFormData({
				...
				apartment:
					loading || !resume.address ? '' : resume.address.apartment,
				...
				instagram:
					loading || !resume.social ? '' : resume.social.instagram,
				linkedin:
					loading || !resume.social ? '' : resume.social.linkedin,
...
export default connect(mapStateToProps, { createResume })(EditResume);
